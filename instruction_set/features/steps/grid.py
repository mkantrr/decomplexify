...
def step_impl(context, width, height):
  ...
  for row in range(int(height)):
    for col in range(int(width)):
      model_dict_update[f"iteration over the agent's neighborhood at ({row}, {col})"] = globals.MODEL.grid.iter_neighborhood(pos=(row, col), moore=False)
      model_dict_update[f"iteration over the agent's moore neighborhood at ({row}, {col})"] = globals.MODEL.grid.iter_neighborhood(pos=(row, col), moore=True)
      model_dict_update[f"the agent's neighborhood at ({row}, {col})"] = globals.MODEL.grid.get_neighborhood(pos=(row, col), moore=False)
      model_dict_update[f"the agent's moore neighborhood at ({row}, {col})"] = globals.MODEL.grid.get_neighborhood(pos=(row, col), moore=True)
      model_dict_update[f"iteration of the agent's neighbors at ({row}, {col})"] = globals.MODEL.grid.iter_neighbors(pos=(row, col), moore=False)
      model_dict_update[f"iteration of the agent's moore neighbors at ({row}, {col})"] = globals.MODEL.grid.iter_neighbors(pos=(row, col), moore=True)
      model_dict_update[f"the agent's neighbors at ({row}, {col})"] = globals.MODEL.grid.get_neighbors(pos=(row, col), moore=False)
      model_dict_update[f"the agent's moore neighbors at ({row}, {col})"] = globals.MODEL.grid.get_neighbors(pos=(row, col), moore=True)
      model_dict_update[f"the list of all cell contents at ({row}, {col})"] = globals.MODEL.grid.get_cell_list_contents((row, col))
      model_dict_update[f"the cell at ({row}, {col}) is empty"] = globals.MODEL.grid.is_cell_empty(pos=(row, col))
      func_write_update[f"iteration over the agent's neighborhood at ({row}, {col})"] = "globals.MODEL.grid.iter_neighborhood(pos=(row, col), moore=False)"
      func_write_update[f"iteration over the agent's moore neighborhood at ({row}, {col})"] = "globals.MODEL.grid.iter_neighborhood(pos=(row, col), moore=True)"
      func_write_update[f"the agent's neighborhood at ({row}, {col})"] = "globals.MODEL.grid.get_neighborhood(pos=(row, col), moore=False)"
      func_write_update[f"the agent's moore neighborhood at ({row}, {col})"] = "globals.MODEL.grid.get_neighborhood(pos=(row, col), moore=True)"
      func_write_update[f"iteration of the agent's neighbors at ({row}, {col})"] = "globals.MODEL.grid.iter_neighbors(pos=(row, col), moore=False)"
      func_write_update[f"iteration of the agent's moore neighbors at ({row}, {col})"] = "globals.MODEL.grid.iter_neighbors(pos=(row, col), moore=True)"
      func_write_update[f"the agent's neighbors at ({row}, {col})"] = "globals.MODEL.grid.get_neighbors(pos=(row, col), moore=False)"
      func_write_update[f"the agent's moore neighbors at ({row}, {col})"] = "globals.MODEL.grid.get_neighbors(pos=(row, col), moore=True)"
      func_write_update[f"the list of all cell contents at ({row}, {col})"] = "globals.MODEL.grid.get_cell_list_contents((row, col))"
      func_write_update[f"the cell at ({row}, {col}) is empty"] = "globals.MODEL.grid.is_cell_empty(pos=(row, col))"
  globals.model_dict.update(model_dict_update)
  globals.func_write.update(func_write_update)
