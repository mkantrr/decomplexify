from globals import runtime_context, MODEL, model_dict
...
@when(u'the model has a defined continuous space sized by width {width:S} and height {height:S}')
def step_impl(context, width, height):
  MODEL.grid = ContinuousSpace(width=int(width), height=int(height), torus=False)
  assert isinstance(MODEL, AgentBasedModel) and isinstance(MODEL.grid, ContinuousSpace)
  
@when(u'the model has a defined continuous space sized by width {width:S} and height {height:S} as torus')
def step_impl(context, width, height):
  MODEL.grid = ContinuousSpace(width=int(width), height=int(height), torus=True)
  assert isinstance(MODEL, AgentBasedModel) and isinstance(MODEL.grid, ContinuousSpace)
  
@when(u'{num_agents:S} {agent_name:S} agents are placed on the grid')
